...
def get_embeddings(text, tokenizer=bert_tokenizer, embeddings_matrix=bert_matrix_embs, method='mean'):
...
def build_databases(df_path=DF_PATH, annoy_path=ANNOY_PATH, df_filtered_path=DF_FILTERED_PATH):
    # Filter and save df
    movies_dataframe = pd.read_csv(df_path, low_memory=False)
    movies_dataframe = movies_dataframe.filter(['title', 'overview', 'id', 'original_language'])
